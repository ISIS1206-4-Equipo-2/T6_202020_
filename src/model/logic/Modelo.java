...
public class Modelo {
	...
	public void estacionesCriticas() {
		...
		for (Vertex<Integer, Estacion> vertex : lista) {
			...
			if (vertex.getInfo().darViajesLlegada() + vertex.getInfo().darViajesSalida() < condU2
					&& !vertex.getInfo().darNombre().equals(vCondU1)) {
		...
		for (Vertex<Integer, Estacion> vertex : lista) {
			if (vertex.getInfo().darViajesLlegada() > condM3 && !vertex.getInfo().darNombre().equals(vCondM1)
					&& !vertex.getInfo().darNombre().equals(vCondM2)) {
				vCondM3 = vertex.getInfo().darNombre();
				condM3 = vertex.getInfo().darViajesLlegada();
			}

			if (vertex.getInfo().darViajesSalida() > condS3 && !vertex.getInfo().darNombre().equals(vCondS1)
					&& !vertex.getInfo().darNombre().equals(vCondS2)) {
				vCondS3 = vertex.getInfo().darNombre();
				condS3 = vertex.getInfo().darViajesSalida();
			}

			if (vertex.getInfo().darViajesLlegada() + vertex.getInfo().darViajesSalida() < condU3
					&& !vertex.getInfo().darNombre().equals(vCondU1) && !vertex.getInfo().darNombre().equals(vCondU2)) {
	...
	/**
	 * Retorna una tupla con dos estaciones, la primera es la que m�s salidas por la
	 * edad tiene, la segunda es la que mas entradas por edad tiene.
	 * 
	 * @param edad
	...
	/**
	 * M�todo escrito por Santiago Bobadilla Modificado por el Grupo 2
	 */
	public void maps() {
	...
	/**
	 * Encuentra los puntos de inicio y fin mas cercanos (id de las estaciones) a
	 * las coordenas de inicio y fin de la ruta Posicion 0: Vertice inicial -
	 * Posicion 1: Vertice final
	 */
	...
	public void losCaminosDeLaVida(Vertex<Integer, Estacion> from, Vertex<Integer, Estacion> to) {
		...
		if (hola != null && djk.distTo(to) != Double.POSITIVE_INFINITY && !(djk.distTo(to) == 0)) {
			...
			for (Edge<Integer, Estacion> edge : hola) {
				System.out.println("\nDESDE " + edge.getSource().getInfo().darNombre() + " HASTA "
						+ edge.getDest().getInfo().darNombre());
			}
	 ...
	 * 
	 * @param edad: "0" 0-10, "1" 11-20, "2" 21-30, "3" 31-40, "4" 41-50, "5" 51-60,
	 *              "6" 60+
	 */
